    ...
    <BaseCard className="px-4 py-3 w-full flex flex-col gap-1 justify-center">
      ...
      {audioUrl && (
        <div className="flex items-center gap-2">
          <button
            className="p-1 -m-1 text-slate-600 dark:text-slate-300 forest:text-gray-300"
            onClick={handleAudio}
          >
            {!audioUrl ? (
              <VolumeOff
                strokeWidth={2.2}
                size={20}
                className="text-gray-400 forest:text-gray-500"
              />
            ) : (
              <Volume2 strokeWidth={2.2} size={20} />
            )}
          </button>
          <span className="italic text-sm text-slate-700 dark:text-slate-400 forest:text-gray-400">
            {phoneticText}
          </span>
        </div>
      )}
